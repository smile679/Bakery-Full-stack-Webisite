...
function Cart() {
  ...
  function handleQuantity({ productId ,payload, quantity }) {
    ...
    if (payload === "plus") {
      dispatch(updateCartItem({ userId: user?.id, productId, quantity : quantity + 1 })).then(data=>{
        if(data.payload?.success){
          dispatch(fetchCartItems({ userId: user?.id }));
        }
      }
  ...
  return (
    <section className="w-full min-h-screen py-15 px-2">
      <h1 className="w-full text-3xl font-bold font-sansita-swashed mt-10 text-center">Your Cart</h1>
      <div className="max-w-6xl mx-auto flex max-md:flex-col py-10 gap-5">
        ...
        <div className="flex-1 w-full max-w-3xl bg-amber-700">123</div>
      </div>
    </section>
  );
}
